...
TS_FP<-TS_FP[,(-1:-2)]
TS_FP<-TS_FP[,(-215:-216)] #remove 'valve' and 'measurement time' columns at the end
...
ncomps = c(1:10)
param="TMn_mgl" #the nutrient you want to find the number of comps for 
...
TS_conc$uncerTMn_max <- WQP_TS + 1.96*sd(as.numeric(fit$residuals[,,10])) #max uncert
TS_conc$uncerTMn_min <- WQP_TS - 1.96*sd(as.numeric(fit$residuals[,,10])) #min uncert
...
png("MUX_TMn_Oct_Nov_2020_pred.png",width = 9, height = 4, units = 'in', res = 300) 
TMn_plot <- ggplot() +
  geom_path(data=TS_conc, aes(x=DateTime,y=TMn_mgl, color= Depth), size=0.5) +
  #geom_ribbon(data=TS_conc, aes(ymin=uncerTMn_min, ymax=uncerTMn_max, x=DateTime, fill = "band"), alpha = 0.2)+
  #geom_point(data=dataWQ, aes(x=Date, y=TMn_mgl, colour= as.factor(Depth))) +
  ylim(0, 4)+
  labs(x="Date", y = "TMn_mgl", title = "Predicted TMn") +
...
png("MUX_TMn_2020_pred_five_m.png",width = 9, height = 4, units = 'in', res = 300) 
TMn_plot <- ggplot() +
  geom_path(data=five_m, aes(x=DateTime,y=TMn_mgl), size=0.5) +
  #geom_ribbon(data=TS_conc, aes(ymin=uncerTMn_min, ymax=uncerTMn_max, x=DateTime, fill = "band"), alpha = 0.2)+
  geom_point(data=five_m_WQ, aes(x=Date, y=TMn_mgl), colour="blue") +
  ylim(0, 7)+
  labs(x="Date", y = "TMn_mgl", title = "Predicted TMn at 5.0m, 2020") +
  scale_x_datetime(labels = date_format("%Y-%m-%d"))+
