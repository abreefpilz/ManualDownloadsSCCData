...
PLSR_SCAN_boot<-function(param,dataCalFP,dataWQ,TS_FP,ncomp,yesplot=FALSE){
  ...
  # Y values (WQP_star)
  WQP_star = WQP + sample(resid(fit)[,,4],nrow(WQP),replace = TRUE)
  WQP_star = data.matrix(WQP_star[1])
  
  # Sample the residuals of the original fitted model to construct a set of new 
  # Predicted values
  WQP_TS_star = WQP_TS + sample(resid(fit)[,,4], nrow(WQP_TS), replace = TRUE)
  ...
  pred_int = apply(G, 1, quantile, c(0.1,0.9))
