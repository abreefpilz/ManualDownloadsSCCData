...
#*****************************************************************
#* TITLE:   MUX 2021 Data Prep and Overlaps Script
#*                     
...
muxfiles<-list.files(path="./MUX/FP File/2021", pattern = ".FP")
...
for(i in 1:length(muxfiles)){ #reads in all files within folder in Github
  if(file.size(paste0("./MUX/FP File/2021/",muxfiles[i]))>4000){
    temp<-read.table(file=paste0("./MUX/FP File/2021/",muxfiles[i]),skip=2,header=FALSE, row.names = NULL, sep = "\t")
