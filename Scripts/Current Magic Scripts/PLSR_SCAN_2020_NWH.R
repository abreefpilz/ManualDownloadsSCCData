...
#Remove first two rows because they correspond to sampling times prior to SSCAN deployment
dataCalFP = dataCalFP[-c(1:2),]
...
TS_FP = TS_FP[TS_FP$Date>"2020-10-15 12:00",]
TS_FP = TS_FP[TS_FP$Date<"2020-11-09 12:00",]
...
TS_conc$uncerTFe_max <- WQP_TS + 1.96*sd(as.numeric(fit$residuals[,,8])) #max uncert
TS_conc$uncerTFe_min <- WQP_TS - 1.96*sd(as.numeric(fit$residuals[,,8])) #min uncert
...
#Subset WQ data for plotting
WQ_plot = dataWQ[dataWQ$Date>"2020-10-15 12:00",]

png("SSCAN_TFe_2020_pred.png",width = 9, height = 4, units = 'in', res = 300) 
TFe_plot <- ggplot() +
  ...
  geom_point(data=WQ_plot, aes(x=Date, y=TFe_mgl), colour="blue") +
  #ylim(-5, 50)+
  labs(x="Date", y = "TFe_mgl", title = "SSCAN Predicted TFe at 1.6m, 2020") +
